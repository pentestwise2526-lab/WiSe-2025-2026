...
2. Perform nmap scan on the found IP by also enabling the default scripts: `nmap -sV -sC -T5 -p- [dc31_01IP]`. Output will mention that redis server is running on port 6379 with version `Redis key-value store 5.0.7`
